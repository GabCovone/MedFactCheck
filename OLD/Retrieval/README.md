...
Il modulo di Retrieval costituisce il nucleo di Information Retrieval (IR) ad alte prestazioni del sistema Multi-Agente **MedFactCheck**. Il suo obiettivo primario è l'estrazione di evidenze cliniche dense, accurate e prive di rumore a partire da due sorgenti distinte: il corpus statico locale **SciFact** (Ramo KB) e la letteratura medica mondiale aggiornata live tramite l'API di **Europe PMC** (Ramo LIT).
